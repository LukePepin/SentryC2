...
def save_results(ecc_metrics: PerformanceMetrics,
                 zkp_metrics: PerformanceMetrics,
                 results: dict) -> str:
    """Save benchmark results to a timestamped file. Returns the filepath."""
    import platform
    ...
    filename = f"h2_results_{hostname}_{timestamp}.txt"
    filepath = os.path.join(script_dir, filename)

    # Capture the printed output into the file
    import io
    buf = io.StringIO()
    ...
    with open(filepath, 'w') as f:
        f.write(f"# H2 Security Tax Benchmark Results\n")
...
def main() -> None:
    ...
    try:
        ...
        filepath = save_results(ecc_metrics, zkp_metrics, results)
